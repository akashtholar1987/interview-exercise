...
The Listener monitors a firehose of events from the Price Source. The Price Source is an async process and it requires some additional state. Hence, the SourceImpl has some state variables.
...
The Price Listener monitors the events, strikes a trade when there is price matching a given criteria
